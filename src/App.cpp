...
App::App()
{
    ...
    m_window = SDL_CreateWindow("Dear ImGui SDL3+SDL_Renderer example", (int)(m_width * main_scale),
                                (int)(m_height * main_scale), window_flags);
    if (m_window == nullptr)
    {
    ...
    // io.ConfigDpiScaleFonts = true;        // [Experimental] Automatically
    ...
    // io.ConfigDpiScaleViewports = true;    // [Experimental] Scale Dear ImGui
    // and Platform Windows when Monitor DPI changes.
    ...
    render::Log::set_callback(
        [](render::LogLevel level, std::string_view msg)
        {
            const char* level_str = "INFO";
            switch (level)
            {
                case render::LogLevel::Debug:
                    level_str = "DEBUG";
                    break;
                case render::LogLevel::Warn:
                    level_str = "WARN";
                    break;
                case render::LogLevel::Error:
                    level_str = "ERROR";
                    break;
            }
    ...
    {
        m_path_tracer =
            render::PathTracer::create_path_tracer(render::PathTracer::BackendType::CPU_EMBREE);
        m_render_scene = std::make_shared<render::Scene>();

        // Random number generator - Ray Tracing in One Weekend style
        std::random_device rd;
        std::mt19937 gen(rd());
        std::uniform_real_distribution<float> random_float(0.0f, 1.0f);

        // Helper to create random diffuse material
        auto createRandomMaterial = [&]()
        {
            render::MaterialDescriptor mat;
            mat.type = render::MaterialType::DIFFUSE;

            // Random diffuse color (darker colors via squaring)
            glm::vec3 albedo = glm::vec3(random_float(gen) * random_float(gen),
                                         random_float(gen) * random_float(gen),
                                         random_float(gen) * random_float(gen));
            mat.parameters["albedo"] = albedo;
        ...
        // Ground sphere
        {
            auto sphere = m_render_scene->CreateNode<render::SphereObject>("Ground");
            sphere->SetRadius(1000.0f);
            sphere->SetPosition(glm::vec3(0.0f, -1000.0f, 0.0f));

            render::MaterialDescriptor groundMat;
            groundMat.type = render::MaterialType::DIFFUSE;
            groundMat.parameters["albedo"] = glm::vec3(0.5f, 0.5f, 0.5f);
            m_render_scene->setMaterial(sphere->GetID(), groundMat);
        }

        // Random small spheres (Ray Tracing in One Weekend style)
        for (int a = -11; a < 11; a++)
        {
            for (int b = -11; b < 11; b++)
            {
                glm::vec3 center(a + 0.9f * random_float(gen), 0.2f, b + 0.9f * random_float(gen));

                // Skip if too close to the large spheres we'll add
                if (glm::length(center - glm::vec3(4.0f, 0.2f, 0.0f)) > 0.9f &&
                    glm::length(center - glm::vec3(-4.0f, 0.2f, 0.0f)) > 0.9f &&
                    glm::length(center - glm::vec3(0.0f, 0.2f, 0.0f)) > 0.9f)
                {
                    auto sphere = m_render_scene->CreateNode<render::SphereObject>("SmallSphere");
                    sphere->SetRadius(0.2f);
                    sphere->SetPosition(center);
                    m_render_scene->setMaterial(sphere->GetID(), createRandomMaterial());
                }
            }
        }
        ...
        {
            // Left
            auto sphere = m_render_scene->CreateNode<render::SphereObject>("Large1");
            sphere->SetRadius(1.0f);
            sphere->SetPosition(glm::vec3(-4.0f, 1.0f, 0.0f));

            render::MaterialDescriptor mat;
            mat.type = render::MaterialType::DIFFUSE;
            mat.parameters["albedo"] = glm::vec3(0.7f, 0.6f, 0.5f);
            m_render_scene->setMaterial(sphere->GetID(), mat);
        }

        {
            // Center
            auto sphere = m_render_scene->CreateNode<render::SphereObject>("Large2");
            sphere->SetRadius(1.0f);
            sphere->SetPosition(glm::vec3(0.0f, 1.0f, 0.0f));

            render::MaterialDescriptor mat;
            mat.type = render::MaterialType::DIFFUSE;
            mat.parameters["albedo"] = glm::vec3(0.4f, 0.2f, 0.1f);
            m_render_scene->setMaterial(sphere->GetID(), mat);
        }

        {
            // Right
            auto sphere = m_render_scene->CreateNode<render::SphereObject>("Large3");
            sphere->SetRadius(1.0f);
            sphere->SetPosition(glm::vec3(4.0f, 1.0f, 0.0f));

            render::MaterialDescriptor mat;
            mat.type = render::MaterialType::DIFFUSE;
            mat.parameters["albedo"] = glm::vec3(0.8f, 0.8f, 0.8f);
            m_render_scene->setMaterial(sphere->GetID(), mat);
        }
    ...
    m_materials_panel = std::make_unique<ui::MaterialsPanel>(m_render_scene, &m_selected_material);
    m_scene_hierarchy_panel =
        std::make_unique<ui::SceneHierarchyPanel>(m_render_scene, &m_selected_node_id);
    m_properties_panel = std::make_unique<ui::PropertiesPanel>(m_render_scene, &m_selected_node_id);
}
...
void App::run()
{
    ...
    while (!done)
    {
        ...
        // and hide them from your application based on those two flags. [If
        // using SDL_MAIN_USE_CALLBACKS: call ImGui_ImplSDL3_ProcessEvent() from
        // your SDL_AppEvent() function]
        SDL_Event event;
...

        {
            ...
            if (new_viewport_dimensions.x != m_viewport_dimensions.x ||
                new_viewport_dimensions.y != m_viewport_dimensions.y)
            {
                m_viewport_dimensions = new_viewport_dimensions;

                // Recreate render target with new size - Embree only
                // m_render_target =
                // RenderTargetFactory::create((int)m_viewport_dimensions.x,
...

            {
                ...
                if (result.width > 0 && result.height > 0)
                {
                    SDL_UpdateTexture((SDL_Texture*)test_tex->get_texture(), nullptr,
                                      result.image_buffer.data(), result.width * sizeof(uint32_t));
                }
            }
...

        {
            ...
            const char* viewport_modes[] = {"Window Dimensions", "Custom 256x256", "Custom 512x512",
                                            "Custom 1024x1024"};
        ...
        m_materials_panel->render();
        renderMaterialPanel();

        // Rendering
        ImGui::Render();
        SDL_SetRenderScale(GraphicsContext::getSDLRenderer(), io.DisplayFramebufferScale.x,
                           io.DisplayFramebufferScale.y);
        SDL_SetRenderDrawColorFloat(GraphicsContext::getSDLRenderer(), m_clear_color.x,
                                    m_clear_color.y, m_clear_color.z, m_clear_color.w);
        SDL_RenderClear(GraphicsContext::getSDLRenderer());
        ImGui_ImplSDLRenderer3_RenderDrawData(ImGui::GetDrawData(),
                                              GraphicsContext::getSDLRenderer());
        SDL_RenderPresent(GraphicsContext::getSDLRenderer());
    }
}

void App::renderMaterialPanel()
{
    ...
    else
    {
        ...
        switch (m_selected_material->type)
        {
        ...
        // Shader path
        if (!m_selected_material->shaderPath.empty())
        {
            ImGui::Text("Shader: %s", m_selected_material->shaderPath.c_str());
        }

        ImGui::Separator();
        ImGui::Text("Parameters:");

        // Edit albedo if it exists
        if (m_selected_material->parameters.find("albedo") != m_selected_material->parameters.end())
        {
            auto& param = m_selected_material->parameters["albedo"];
            if (auto* color = std::get_if<glm::vec3>(&param))
            {
                float col[3] = {color->r, color->g, color->b};
                if (ImGui::ColorEdit3("Albedo", col))
                {
                    *color = glm::vec3(col[0], col[1], col[2]);
                    // Mark scene as dirty so renderer knows to rebuild
                    m_render_scene->markDirty();
                }
            }
        }

        // Edit roughness if it exists
        if (m_selected_material->parameters.find("roughness") !=
            m_selected_material->parameters.end())
        {
            auto& param = m_selected_material->parameters["roughness"];
            if (auto* roughness = std::get_if<float>(&param))
            {
        ...
        ImGui::Separator();
        ImGui::Text("All Parameters (%zu):", m_selected_material->parameters.size());
        for (const auto& [key, value] : m_selected_material->parameters)
        {
            std::visit(
                [&key](const auto& v)
                {
...
static void SetDarkThemeColors()
{
    ...
    // Base colors
    colors[ImGuiCol_Text] = ImVec4(0.80f, 0.80f, 0.80f,
                                   1.00f);  // Light grey text (not pure white)
    ...
    colors[ImGuiCol_FrameBgHovered] =
        ImVec4(0.20f, 0.20f, 0.20f, 0.95f);  // Slightly lighter on hover
    colors[ImGuiCol_FrameBgActive] =
        ImVec4(0.24f, 0.24f, 0.24f, 1.00f);  // Even lighter when active

    // Title bar colors
    colors[ImGuiCol_TitleBg] = ImVec4(0.15f, 0.15f, 0.15f, 1.00f);  // Dark grey inactive title
    colors[ImGuiCol_TitleBgActive] =
        ImVec4(0.18f, 0.18f, 0.18f, 1.00f);  // Slightly lighter active title
    colors[ImGuiCol_TitleBgCollapsed] =
        ImVec4(0.15f, 0.15f, 0.15f, 0.75f);                           // Transparent when collapsed
    ...
    colors[ImGuiCol_ScrollbarGrabHovered] =
        ImVec4(0.30f, 0.30f, 0.30f, 1.00f);  // Scrollbar grab when hovered
    colors[ImGuiCol_ScrollbarGrabActive] =
        ImVec4(0.35f, 0.35f, 0.35f, 1.00f);  // Scrollbar grab when active
    ...
    colors[ImGuiCol_SliderGrabActive] =
        ImVec4(0.40f, 0.40f, 0.40f, 1.00f);                        // Slider grab when active
    colors[ImGuiCol_Button] = ImVec4(0.20f, 0.20f, 0.20f, 0.80f);  // Dark grey buttons
    colors[ImGuiCol_ButtonHovered] =
        ImVec4(0.25f, 0.25f, 0.25f, 1.00f);  // Slightly lighter on hover
    ...
    colors[ImGuiCol_HeaderHovered] =
        ImVec4(0.25f, 0.25f, 0.25f, 0.80f);  // Slightly lighter on hover
    ...
    colors[ImGuiCol_SeparatorHovered] =
        ImVec4(0.30f, 0.30f, 0.30f, 1.00f);  // Separator when hovered
    ...
    colors[ImGuiCol_ResizeGripHovered] =
        ImVec4(0.30f, 0.30f, 0.30f, 0.75f);  // Resize grip when hovered
    colors[ImGuiCol_ResizeGripActive] =
        ImVec4(0.35f, 0.35f, 0.35f, 1.00f);  // Resize grip when active
    ...
    colors[ImGuiCol_TabSelectedOverline] =
        ImVec4(0.22f, 0.22f, 0.22f, 1.00f);                           // Selected tab overline
    colors[ImGuiCol_TabDimmed] = ImVec4(0.13f, 0.13f, 0.13f, 0.86f);  // Dimmed/unfocused tab
    colors[ImGuiCol_TabDimmedSelected] =
        ImVec4(0.18f, 0.18f, 0.18f, 1.00f);  // Selected but unfocused tab
    colors[ImGuiCol_TabDimmedSelectedOverline] =
        ImVec4(0.18f, 0.18f, 0.18f, 1.00f);  // Overline of unfocused selected tab
    ...
    colors[ImGuiCol_TabUnfocusedActive] =
        ImVec4(0.18f, 0.18f, 0.18f, 1.00f);  // Unfocused active tab (old name)
    ...
    colors[ImGuiCol_PlotLinesHovered] =
        ImVec4(0.70f, 0.70f, 0.70f, 1.00f);                               // Plot lines when hovered
    colors[ImGuiCol_PlotHistogram] = ImVec4(0.60f, 0.60f, 0.60f, 1.00f);  // Plot histogram
    colors[ImGuiCol_PlotHistogramHovered] =
        ImVec4(0.70f, 0.70f, 0.70f, 1.00f);  // Plot histogram when hovered
    ...
    colors[ImGuiCol_TableRowBg] =
        ImVec4(0.14f, 0.14f, 0.14f, 0.90f);  // Table row background (even)
    colors[ImGuiCol_TableRowBgAlt] =
        ImVec4(0.16f, 0.16f, 0.16f, 0.90f);  // Table row background (odd)

    // Miscellaneous
    colors[ImGuiCol_TextLink] =
        ImVec4(0.55f, 0.55f, 0.55f, 1.00f);  // Light grey for links (not blue)
    colors[ImGuiCol_TextSelectedBg] =
        ImVec4(0.30f, 0.30f, 0.30f, 0.35f);  // Light grey selection background
    ...
    colors[ImGuiCol_NavWindowingHighlight] =
        ImVec4(0.40f, 0.40f, 0.40f, 0.70f);  // Nav windowing highlight
